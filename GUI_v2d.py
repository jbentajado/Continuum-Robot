...
- add arduino actuation
- turn imu readings into vars
'''
...
def run():
    if running:
        ...
        if list_check != no:
            ...
            #if x != 0 or y != 0:
            if x > 5 or x < -5 or y > 5 or y < -5: # Centering deadzone
                move_motors(x, y)
                # blink()
...
ar1_rely = 0.15       
ar2_rely = 0.3        # row 1: motor 1 info
ar3_rely = 0.45       # row 2: motor 2 info
ar4_rely =  0.6       # row 3: motor 3 info
ar5_rely = 0.75       # row 4: motor 4 info
ar6_rely = 0.9        # row 5: buttons row
ac1_relx = 0.2        # column 1: position
...
position_label = tk.Label(middle_frame, text = 'position (pulse)', font = myFont)
position_label.place(relx = ac1_relx, rely = ar1_rely, 
...
motor1_label = tk.Label(middle_frame, text = 'motor 1:', font = myFont)
motor1_label.place(rely = ar2_rely, 
                   relwidth = ac0_relwidth, relheight = a_relheight)

motor2_label = tk.Label(middle_frame, text = 'motor 2:', font = myFont)
motor2_label.place(rely = ar3_rely, 
                   relwidth = ac0_relwidth, relheight = a_relheight)
...
run_button = Button(middle_frame, text = "Run", 
                  command=manual_entry, bg = 'green', font = myFont)
